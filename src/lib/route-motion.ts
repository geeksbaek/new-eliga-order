 ...
 */
export type MainTab = 'home' | 'dining' | 'cafe' | 'cart' | 'orders'
...
export function mainTabFromPath(path: string): MainTab {
  ...
  if (path.startsWith('/cart') || path.startsWith('/order')) return 'cart'
  if (path.startsWith('/cafe')) return 'cafe'
  return 'home'
}

export const MAIN_TAB_ORDER: MainTab[] = [
  'home',
  'dining',
  'cafe',
  'cart',
